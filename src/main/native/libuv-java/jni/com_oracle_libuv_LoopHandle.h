/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class com_oracle_libuv_LoopHandle */

#ifndef _Included_com_oracle_libuv_LoopHandle
#define _Included_com_oracle_libuv_LoopHandle
#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     com_oracle_libuv_LoopHandle
 * Method:    _new
 * Signature: ()J
 */
JNIEXPORT jlong JNICALL Java_com_oracle_libuv_LoopHandle__1new
  (JNIEnv *, jclass);

/*
 * Class:     com_oracle_libuv_LoopHandle
 * Method:    _static_initialize
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_com_oracle_libuv_LoopHandle__1static_1initialize
  (JNIEnv *, jclass);

/*
 * Class:     com_oracle_libuv_LoopHandle
 * Method:    _run
 * Signature: (JI)I
 */
JNIEXPORT jint JNICALL Java_com_oracle_libuv_LoopHandle__1run
  (JNIEnv *, jobject, jlong, jint);

/*
 * Class:     com_oracle_libuv_LoopHandle
 * Method:    _stop
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_com_oracle_libuv_LoopHandle__1stop
  (JNIEnv *, jobject, jlong);

/*
 * Class:     com_oracle_libuv_LoopHandle
 * Method:    _destroy
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_com_oracle_libuv_LoopHandle__1destroy
  (JNIEnv *, jobject, jlong);

/*
 * Class:     com_oracle_libuv_LoopHandle
 * Method:    _close_all
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_com_oracle_libuv_LoopHandle__1close_1all
  (JNIEnv *, jobject, jlong);

/*
 * Class:     com_oracle_libuv_LoopHandle
 * Method:    _list
 * Signature: (J)[Ljava/lang/String;
 */
JNIEXPORT jobjectArray JNICALL Java_com_oracle_libuv_LoopHandle__1list
  (JNIEnv *, jobject, jlong);

/*
 * Class:     com_oracle_libuv_LoopHandle
 * Method:    _get_last_error
 * Signature: (J)Lcom/oracle/libuv/NativeException;
 */
JNIEXPORT jthrowable JNICALL Java_com_oracle_libuv_LoopHandle__1get_1last_1error
  (JNIEnv *, jobject, jlong);

/*
 * Class:     com_oracle_libuv_LoopHandle
 * Method:    _set_last_error
 * Signature: (JI)V
 */
JNIEXPORT void JNICALL Java_com_oracle_libuv_LoopHandle__1set_1last_1error
  (JNIEnv *, jobject, jlong, jint);

#ifdef __cplusplus
}
#endif
#endif
