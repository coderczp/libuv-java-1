/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class com_oracle_libuv_ProcessHandle */

#ifndef _Included_com_oracle_libuv_ProcessHandle
#define _Included_com_oracle_libuv_ProcessHandle
#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     com_oracle_libuv_ProcessHandle
 * Method:    _new
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_com_oracle_libuv_ProcessHandle__1new
  (JNIEnv *, jclass, jlong);

/*
 * Class:     com_oracle_libuv_ProcessHandle
 * Method:    _static_initialize
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_com_oracle_libuv_ProcessHandle__1static_1initialize
  (JNIEnv *, jclass);

/*
 * Class:     com_oracle_libuv_ProcessHandle
 * Method:    _initialize
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_com_oracle_libuv_ProcessHandle__1initialize
  (JNIEnv *, jobject, jlong);

/*
 * Class:     com_oracle_libuv_ProcessHandle
 * Method:    _spawn
 * Signature: (JLjava/lang/String;[Ljava/lang/String;[Ljava/lang/String;Ljava/lang/String;I[I[J[III)I
 */
JNIEXPORT jint JNICALL Java_com_oracle_libuv_ProcessHandle__1spawn
  (JNIEnv *, jobject, jlong, jstring, jobjectArray, jobjectArray, jstring, jint, jintArray, jlongArray, jintArray, jint, jint);

/*
 * Class:     com_oracle_libuv_ProcessHandle
 * Method:    _close
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_com_oracle_libuv_ProcessHandle__1close
  (JNIEnv *, jobject, jlong);

/*
 * Class:     com_oracle_libuv_ProcessHandle
 * Method:    _kill
 * Signature: (JI)I
 */
JNIEXPORT jint JNICALL Java_com_oracle_libuv_ProcessHandle__1kill
  (JNIEnv *, jobject, jlong, jint);

#ifdef __cplusplus
}
#endif
#endif
